...
interface CitationMarkerRecord {
  marker: string;
  evidenceId: string;
  citationId: string;
  citation: {
...
type CitationMarkerMap = Record<string, CitationMarkerRecord>;
...
interface Message {
  ...
  text: string;
  isStreaming?: boolean;
  // Application-owned Change 13 citation marker mapping retained with the
  // assistant message so [S1] in the model output can be resolved without
  // trusting any citation metadata generated by the model.
  citationMarkers?: CitationMarkerMap;
...
const NativelyInterface: React.FC<NativelyInterfaceProps> = ({
  ...
  // then.
  const pendingFinalizeRef = useRef<{ msgId: string; intent: string; text: string; citationMarkers?: CitationMarkerMap } | null>(null);
  ...
  // not smooth reveal.
  const finalizeWhenRevealCaughtUp = useCallback((msgId: string, intent: string, finalText: string, citationMarkers?: CitationMarkerMap) => {
    streamingTextRef.current = finalText;
    pendingFinalizeRef.current = { msgId, intent, text: finalText, citationMarkers };
    if (pendingFinalizeTimeoutRef.current !== null) {
      clearTimeout(pendingFinalizeTimeoutRef.current);
    }
    ...
    pendingFinalizeTimeoutRef.current = setTimeout(() => {
      ...
      streamingRenderModeRef.current = 'imperative';
      setMessages((prev) => {
        const committed = commitStreamingFlush(prev, pending.msgId, pending.text);
        if (!pending.citationMarkers) return committed;
        return committed.map((message) =>
          message.id === pending.msgId
            ? { ...message, citationMarkers: pending.citationMarkers }
            : message,
        );
      });
    }, safetyNetMs);
    ...
    cleanups.push(
      window.electronAPI.onGeminiStreamDone((data) => {
        ...
        if (
          ...
          finalizeWhenRevealCaughtUp(pendingMsgIdSnapshot, 'chat', authoritativeText, citationMarkers);
          return;
        }
        ...
        setMessages((prev) => {
          ...
          if (target && target.role === 'system') {
            ...
            if (idx !== -1) {
              const updated = [...prev];
              updated[idx] = { ...target, text, isStreaming: false, isCode, ...(citationMarkers ? { citationMarkers } : {}) };
              return updated;
            }
            return [...prev.slice(0, -1), { ...target, text, isStreaming: false, isCode, ...(citationMarkers ? { citationMarkers } : {}) }];
          }
