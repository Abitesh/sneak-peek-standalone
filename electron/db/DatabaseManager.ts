...
// Version 32 → 33: Add embedding storage/metadata for Personal Files.
// Personal vectors use dedicated sqlite-vec tables because personal chunk ids are TEXT
// and must remain isolated from meeting chunk ids.
if (version < 33) {
console.log('[DatabaseManager] Applying migration v32 → v33: Add personal-file embedding columns');
const personalChunkColumns = [
"ALTER TABLE personal_file_chunks ADD COLUMN embedding BLOB",
"ALTER TABLE personal_file_chunks ADD COLUMN embedding_provider TEXT",
"ALTER TABLE personal_file_chunks ADD COLUMN embedding_dimensions INTEGER",
"ALTER TABLE personal_file_chunks ADD COLUMN embedding_space TEXT",
];
for (const sql of personalChunkColumns) {
try { this.db.exec(sql); } catch (_) { /* column already exists */ }
}
try {
this.db.exec(`CREATE INDEX IF NOT EXISTS idx_personal_file_chunks_embedding_space
ON personal_file_chunks(file_id, embedding_space);`);
} catch (_) { /* non-fatal */ }
for (const dim of [384, ...DatabaseManager.KNOWN_DIMS]) {
this.ensurePersonalVecTableForDim(dim);
}
...
/**
* Lazily create a per-dimension sqlite-vec table for Personal File chunks.
* The source table keeps TEXT chunk ids; sqlite-vec stores the stable SQLite rowid.
*/
public ensurePersonalVecTableForDim(dim: number): void {
if (!this.db) return;
if (!Number.isInteger(dim) || dim <= 0 || dim > 100_000) {
console.error(`[DatabaseManager] Invalid personal embedding dimension for vec0 table: ${dim}`);
return;
}
try {
this.db.exec(`
CREATE VIRTUAL TABLE IF NOT EXISTS vec_personal_chunks_${dim} USING vec0(
personal_chunk_rowid INTEGER PRIMARY KEY,
embedding float[${dim}] distance_metric=cosine
);
`);
} catch (e) {
...
try {
const rows = this.db.prepare(
`SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'vec_personal_chunks_%'`
).all() as { name: string }[];
for (const r of rows) {
const m = r.name.match(/^vec_personal_chunks_(\d+)$/);
if (m) dims.add(Number(m[1]));
}
} catch (e) {
console.warn('[DatabaseManager] getExistingPersonalVecDims failed:', e);
}
return [...dims];
}
/**
