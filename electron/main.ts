...
function emergencyCloseDatabase(reason: string): void {
 ...
 // half-initialized process — or interrupted by the macOS SIGTERMSIGKILL
...
type STTProvider = (GoogleSTT | LocalWhisperSTT | RestSTT | DeepgramStreamingSTT | SonioxStreamingSTT | ElevenLabsStreamingSTT | OpenAIStreamingSTT | NativelyProSTT | NvidiaNimStreamingSTT) & {
 finalize?: () => void;
...
export class AppState {
 ...
 // Without this guard, a rapid stopstartstop cycle could enqueue the same
 ...
 private _listenAudioActive: boolean = false;
 ...
 // one-motion YEnter flow) waits for delivery instead of racing it.
 this.sendToWindow(
 ...
 .map((m: any) => ({
 type: m.type,
 rssMB: m.memory?.workingSetSize ? Math.round(m.memory.workingSetSize / 1024) : 0, // KBMB
 pid: m.pid,
 ...
 */
 ...
 // space yet). Without this explicit kick, a v1v2 model upgrade would leave the
 ...
 */
 ...
 if (this.isMeetingActive && this._listenAudioActive) {
 ...
 */
 private startCaptureChannels(context: string, startSystem = this._listenAudioActive): CaptureChannelStartResult {
 ...
 */
 ...
 // still visible, the launcher's CTA pill briefly crossfades bluegreen
 ...
 */
 ...
 // recompute + the shellWidthOS-resize shrink) when the start-side
 ...
 // clear (messages, shellWidthcollapsed, code-expansion refs/timers); the
 ...
 // the normal StopStart path it is now a no-op (state already clean).
 this.sendToWindow(this.getWindowHelper().getOverlayWindow(), 'session-reset');
 ...
 // wrappers in place, a fast StopStart on the SAME device skips the
 ...
 // in-flight teardown as `_pendingTeardown` so a fast startstopstart
 ...
 */
 ...
 // endMeeting() swaps overlaylauncher, the activating show re-shows the tile,
 ...
 // a burst of launcher shows (rapid StopStartStop, or a cold-start
 ...
 */
 ...
 // accessoryregular promotion after createWindow() — see the whenReady block.
 //
 ...
 // duplicate. The old code still bracketed this rename in accessoryregular
...
if (process.env.THINKING_MATRIX === '1') {
 ...
 // renderer's native RSS explodes (4972008MB in ~4s, flat JS heap) within
