...
interface RagIndexStatusSnapshot {
  sourceType: 'mode' | 'personal'; documentId: string; status: RagIndexStatus;
  chunkCount: number; embeddedChunkCount: number; extractedPageCount?: number; totalPageCount?: number;
  errorCode?: string; errorMessage?: string; updatedAt: number;
}

interface RagCitationMarker {
marker: string;
evidenceId: string;
citationId: string;
citation: {
...
ragGetIndexStatus: (sourceType: 'mode' | 'personal', documentId: string) => Promise<{ success: boolean; status?: RagIndexStatusSnapshot; error?: string }>;
modesGetReferenceFileStatus: (
modeId: string,
) => Promise<{ success: boolean; statuses?: Array<{ fileId: string; fileName: string; status: RagIndexStatus; chunkCount: number; embeddedChunkCount: number; extractedPageCount?: number; totalPageCount?: number; errorCode?: string; errorMessage?: string; updatedAt?: number }>; error?: string }>;
onModeFileIndexStatus: (callback: (data: { modeId: string; fileId: string; status: RagIndexStatus; snapshot?: RagIndexStatusSnapshot }) => void) => () => void;
onRagIndexStatus: (callback: (data: RagIndexStatusSnapshot) => void) => () => void;
...
getRagSettings: () => ipcRenderer.invoke('rag-settings:get'),
setRagSetting: (key: string, value: boolean | null) => ipcRenderer.invoke('rag-settings:set', { key, value }),
...
ragGetIndexStatus: (sourceType: 'mode' | 'personal', documentId: string) =>
ipcRenderer.invoke('rag:get-index-status', sourceType, documentId),
modesGetReferenceFileStatus: (modeId: string) =>
ipcRenderer.invoke('modes:get-reference-file-status', modeId),
onModeFileIndexStatus: (callback: (data: { modeId: string; fileId: string; status: RagIndexStatus; snapshot?: RagIndexStatusSnapshot }) => void) => {
const subscription = (_: any, data: any) => callback(data);
...
onRagIndexStatus: (callback: (data: RagIndexStatusSnapshot) => void) => {
const subscription = (_: any, data: RagIndexStatusSnapshot) => callback(data);
ipcRenderer.on('rag-index-status', subscription);
return () => {
ipcRenderer.removeListener('rag-index-status', subscription);
};
},
...
// "What should I say?" should briefly wait for (one-motion YEnter).
