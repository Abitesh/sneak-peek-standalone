...
private writeCallCounter = 0; // Track write() calls for throttled logging
private tickCounter = 0; // Track streamingTick invocations for diagnostics
...
constructor(modelId: string, inputSampleRate: number = 16000) {
super();
this.modelId = modelId;
this.inputSampleRate = inputSampleRate; // Accept sample rate from caller (default 16kHz)
...
console.log(`[LocalWhisperSTT/${this.channelLabel || modelId}] streaming profile: interval=${profile.intervalMs}ms minAudio=${profile.minAudioMs}ms skipAgreement=${profile.skipAgreement} inputRate=${inputSampleRate}Hz`);
}
/**
* Set the actual input sample rate for this STT instance.
* CRITICAL: Incoming audio from SystemAudioCapture/MicrophoneCapture is at the
* declared emitted rate (typically 16000Hz after DSP resampling). This must match
* the rate passed to resampleToF32() or audio will be resampled incorrectly.
*/
setSampleRate(rate: number): void {
if (rate > 0 && rate !== this.inputSampleRate) {
console.log(`[LocalWhisperSTT/${this.channelLabel}] Updated input sample rate: ${this.inputSampleRate}Hz → ${rate}Hz`);
...
// queues final segments until the worker becomes ready
...
// Detect silence/real audio in the chunk
let isAllZeros = true;
let rms = 0;
...
if (Math.abs(sample) > 0.001) isAllZeros = false;
sumSquares += sample * sample;
}
rms = Math.sqrt(sumSquares / samples);
}
// THROTTLED LOGGING: Report cumulative progress instead of every chunk
const writeCounter = ++this.writeCallCounter;
if (writeCounter % 50 === 1) {
const duration = ((chunk.length / 2) / this.inputSampleRate) * 1000;
const audioType = isAllZeros ? 'SILENCE' : `REAL (rms=${(rms * 1000).toFixed(0)})`;
console.log(`[LocalWhisperSTT/${this.channelLabel}] write() chunks=${writeCounter} total=${(chunk.length * writeCounter / 1920).toFixed(1)}KB last_chunk=${chunk.length}B rate=${this.inputSampleRate}Hz (~${duration.toFixed(0)}ms/chunk) type=${audioType}`);
}
// CRITICAL FIX: Skip silence/keepalive frames (all zeros) from Rust DSP.
// Rust silence suppression sends keepalive frames (all zeros) mixed with
// real audio frames. When VAD receives these silence frames interleaved
// with real audio, it cannot detect a continuous speech segment because
// the silence breaks audio continuity. We filter them out here to allow
// VAD to see continuous real audio and correctly detect speech onsets.
if (isAllZeros) {
if (writeCounter % 100 === 1) {
console.log(`[LocalWhisperSTT/${this.channelLabel}] write() #${writeCounter}: SKIPPED silence/keepalive frame (all zeros)`);
}
return; // Skip VAD processing for this frame
}
const f32 = resampleToF32(chunk, this.inputSampleRate);
const before = {
inSpeech: this.vad.isInSpeech(),
open: this.vad.peekOpenSegment()
};
...
const diagMsg = `write() VAD state: before=(inSpeech=${before.inSpeech} openMs=${before.open?.durationMs ?? 'none'})` +
` → after=(inSpeech=${after.inSpeech} openMs=${after.open?.durationMs ?? 'none'}) closed=${after.closed}`;
console.log(`[LocalWhisperSTT/${this.channelLabel}] ${diagMsg}`);
}
segs.forEach((s, i) => {
console.log(`[LocalWhisperSTT/${this.channelLabel}] Closing segment ${i}: ${s.samples.length} samples = ${Math.round((s.samples.length / 16000) * 1000)}ms`);
this.dispatchFinal(s.samples);
});
...
if (committed) {
console.log(`[LocalWhisperSTT/${this.channelLabel}] Soft-commit (MAX_SEGMENT exceeded): ${committed.samples.length} samples`);
...
this.firstPartialEmittedForSegment = 0;
console.log(`[LocalWhisperSTT/${this.channelLabel}] NEW SPEECH SEGMENT OPENED: id=${id}`);
}
}
...
console.log(`[LocalWhisperSTT/${this.channelLabel}] GAP FLUSH triggered: ${pending.length} segments`);
...
const tickDiag = this.writeCallCounter % 50 === 0; // Log every 50 write cycles
// Log tick invocation occasionally for system debugging
this.tickCounter++;
if (this.tickCounter % 100 === 1) {
console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick #${this.tickCounter} called (writeCounter=${this.writeCallCounter})`);
}
if (!this.isActive || !this.vad || !this.workerReady || !this.worker) {
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: NOT READY (active=${this.isActive} vad=${!!this.vad} ready=${this.workerReady} worker=${!!this.worker})`);
this.recordStreamingStall();
return;
}
// Cheap early-return: skip the peekOpenSegment allocation when the
// VAD isn't currently in a speech segment.
const inSpeech = this.vad.isInSpeech();
if (!inSpeech) {
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: NO SPEECH DETECTED`);
this.recordStreamingStall();
return;
}
// Don't stack streaming requests — wait for the previous one to finish.
if (this.streamingTaskInFlight) {
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: TASK IN FLIGHT`);
this.recordStreamingStall();
return;
}
const open = this.vad.peekOpenSegment();
if (!open) {
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: NO OPEN SEGMENT`);
this.recordStreamingStall();
return;
}
...
if (pending < NEMOTRON_CHUNK_SAMPLES) {
// NOT a stall — deliberately does not call recordStreamingStall().
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: INSUFFICIENT NEMOTRON CHUNK (pending=${pending} < ${NEMOTRON_CHUNK_SAMPLES})`);
return;
}
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: DISPATCH Nemotron (pending=${pending} segmentMs=${open.durationMs})`);
} else {
if (open.durationMs < this.streamingMinAudioMs) {
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: TOO SHORT (segmentMs=${open.durationMs} < minAudioMs=${this.streamingMinAudioMs})`);
this.recordStreamingStall();
return;
}
if (tickDiag) console.log(`[LocalWhisperSTT/${this.channelLabel}] streamingTick: DISPATCH (segmentMs=${open.durationMs})`);
}
...
console.log(`[LocalWhisperSTT/${this.channelLabel}] dispatchFinal: SEGMENT FINAL ${audio.length} samples = ${Math.round((audio.length / 16000) * 1000)}ms`);
...
console.log(`[LocalWhisperSTT/${this.channelLabel}] Worker not ready yet; queuing final (pending=${this.pendingAudio.length + 1})`);
...
console.log(`[LocalWhisperSTT/${this.channelLabel}] Sending final: ${outgoing.length} samples`);
...
// Create the Worker shell before waiting for the shared ONNX gate. The
// semaphore may be temporarily held by the Mic Whisper session; keeping the
// Worker reference alive lets this channel queue final segments until its
// inference session can be initialized.
console.log(`[LocalWhisperSTT/${this.channelLabel}] Cold-starting worker shell for ${this.modelId}`);
const workerPath = resolveWhisperWorkerPath();
writeLoadSentinel(this.modelId);
const coldWorker = new Worker(workerPath);
this.worker = coldWorker;
this.attachWorkerListeners();

try {
this.slotRelease = await acquireOnnxSlot('high', 1);
} catch (err) {
if (this.worker === coldWorker) {
this.worker = null;
this.workerReady = false;
}
coldWorker.removeAllListeners('message');
coldWorker.removeAllListeners('error');
coldWorker.removeAllListeners('exit');
await coldWorker.terminate();
clearLoadSentinel(this.modelId);
throw err;
}

// The instance may have been stopped while the ONNX gate was pending. Do not
// initialize a worker that no longer belongs to an active STT session.
if (this.worker !== coldWorker || !this.isActive) {
...
console.log(`[LocalWhisperSTT/${this.channelLabel}] Initializing worker for ${this.modelId}`);
coldWorker.postMessage(buildWorkerInitMessage(this.modelId));
}
...
clearLoadSentinel(this.modelId);
this.workerReady = true;
console.log(`[LocalWhisperSTT/${this.channelLabel}] Worker READY for ${this.modelId}; pending=${this.pendingAudio.length}`);
this.flushPending();
return;
}
