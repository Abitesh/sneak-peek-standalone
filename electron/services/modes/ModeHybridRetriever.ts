...
private chunkCache = new Map<string, { hash: string; chunks: DocumentMapChunk[] }>();
/**
...
['page_start', 'INTEGER'], ['page_end', 'INTEGER'], ['section', 'TEXT'], ['heading', 'TEXT'],
['content_type', "TEXT NOT NULL DEFAULT 'text'"], ['table_index', 'INTEGER'], ['metadata_json', "TEXT NOT NULL DEFAULT '{}'"],
] as const) {
try { this.db.exec(`ALTER TABLE mode_reference_chunks ADD COLUMN ${name} ${definition}`); } catch { /* column exists */ }
}
...
this.persistChunks(file.id, chunkRecords, embeddings, result.space);
...
this.persistChunks(file.id, chunkRecords, embeddedVectors, embeddingSpace);
...
this.persistChunks(file.id, chunkRecords, padded, embeddingSpace);
...
INSERT INTO mode_reference_chunks (file_id, chunk_index, text, embedding, embedding_space, created_at, page_start, page_end, section, heading, content_type, table_index, metadata_json)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
ins.run(fileId, i, chunk.text, blob, vec ? space : null, now, chunk.pageStart ?? null, chunk.pageEnd ?? null, chunk.section ?? null, chunk.heading ?? null, chunk.contentType ?? 'text', chunk.tableIndex ?? null, JSON.stringify(chunk.metadata ?? {}));
}
});
txn();
} catch (e) {
...
let chunks: DocumentMapChunk[];
const cached = this.chunkCache.get(file.id);
if (cached && cached.hash === contentHash) {
chunks = cached.chunks;
} else {
chunks = this.chunkRecords(content);
...
*/
private chunkRecords(content: string): DocumentMapChunk[] {
return buildDocumentChunks(content, { chunkWords: CHUNK_WORDS, chunkOverlap: CHUNK_OVERLAP, tableRowsPerChunk: 10 });
}

private chunkText(content: string): string[] {
return this.chunkRecords(content).map((chunk) => chunk.text);
}
