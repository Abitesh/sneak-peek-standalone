...
function pageAtOffset(source: string, offset: number): number | undefined {
if (offset < 0) return undefined;
let page: number | undefined;
const re = /\[Page\s+(\d+)\]/g;
let match: RegExpExecArray | null;
while ((match = re.exec(source)) !== null && match.index <= offset) page = Number(match[1]);
return page;
}
function tableMetadata(source: string, tableIndex: number, offset: number | null, section?: string, heading?: string, fallbackPage?: number): Record<string, unknown> {
const page = offset === null ? fallbackPage : pageAtOffset(source, offset);
return { table: true, tableIndex, ...(page !== undefined ? { page } : {}), ...(section ? { section } : {}), ...(heading ? { heading } : {}) };
}
...
const offset = findSourceOffset(source, text, searchOffset);
pushChunk(text, {
contentType: 'table',
tableIndex,
metadata: { documentMap: true, ...tableMetadata(source, tableIndex, offset?.start ?? null) },
}, text);
}
return chunks;
}
...
: `[p${section.pageStart}]`;
const isTable = looksLikeTableChunk(window);
...
...(heading ? { heading } : {}),
contentType: isTable ? 'table' : 'section',
...(isTable ? { tableIndex } : {}),
metadata: { documentMap: true, depth: section.depth, containsTable: isTable, ...(isTable ? tableMetadata(source, tableIndex, null, section.num, heading, section.pageStart) : {}) },
}, window);
}
}
...
...(isTable ? { tableIndex } : {}),
metadata: { documentMap: true, containsTable: isTable, ...(isTable ? tableMetadata(source, tableIndex, null, undefined, undefined, block.page) : {}) },
}, window);
}
}
return chunks;
