...
export type RagIndexStatus = 'NOT_INDEXED' | 'QUEUED' | 'EXTRACTING' | 'CHUNKING' | 'EMBEDDING' | 'READY' | 'FAILED' | 'OCR_REQUIRED';
...
const id = params.id || `ref_${crypto.randomUUID()}`;
...
invalidateIndexAttempt('mode', id);
...
public getReferenceFileIndexStatuses(modeId: string): Array<{
fileId: string; fileName: string; status: RagIndexStatus; chunkCount: number;
embeddedChunkCount: number; extractedPageCount?: number; totalPageCount?: number;
errorCode?: string; errorMessage?: string; updatedAt?: number;
}> {
const db = DatabaseManager.getInstance();
return this.getReferenceFiles(modeId).map(file => {
const canonical = db.getRagIndexStatus('mode', file.id);
if (canonical) return {
fileId: file.id,
fileName: file.fileName,
status: canonical.status as RagIndexStatus,
chunkCount: Number(canonical.chunkCount) || 0,
embeddedChunkCount: Number(canonical.embeddedChunkCount) || 0,
extractedPageCount: canonical.extractedPageCount ?? file.extractedPageCount,
totalPageCount: canonical.totalPageCount ?? file.pageCount,
errorCode: canonical.errorCode || undefined,
errorMessage: canonical.errorMessage || undefined,
updatedAt: canonical.updatedAt,
};
const legacy = this.modeContextRetriever.getReferenceFileIndexStatus(file.id);
const mapped: RagIndexStatus = legacy.status === 'ready' ? 'READY'
: legacy.status === 'ocr_required' ? 'OCR_REQUIRED'
: legacy.status === 'indexing' ? 'EMBEDDING'
...
embeddedChunkCount: mapped === 'READY' ? legacy.chunkCount || 0 : 0,
