...
const blob = this.embeddingToBlob(embedding);
this.db.prepare(`
UPDATE personal_file_chunks
SET embedding = ?, embedding_space = ?, embedding_provider = ?, embedding_dimensions = ?
WHERE id = ?
`).run(blob, spaceKey, providerName ?? null, dimensions ?? embedding.length, chunkId);
if (!this.useNativeVec) return;
const dim = embedding.length;
DatabaseManager.getInstance().ensurePersonalVecTableForDim(dim);
try {
const row = this.db.prepare('SELECT rowid FROM personal_file_chunks WHERE id = ?').get(chunkId) as { rowid?: number } | undefined;
if (!row?.rowid) return;
this.db.prepare(`
INSERT OR REPLACE INTO vec_personal_chunks_${dim}(personal_chunk_rowid, embedding) VALUES (?, ?)
`).run(BigInt(row.rowid), blob);
} catch (e) {
console.warn(`[VectorStore] Failed to insert personal embedding into vec_personal_chunks_${dim}:`, e);
}
}

/** Search Personal File embeddings using native sqlite-vec or JS cosine fallback. */
async searchSimilarPersonal(
queryEmbedding: number[],
options: { fileId?: string; limit?: number; minSimilarity?: number; spaceKey?: string } = {},
): Promise<{ chunkId: string; fileId: string; similarity: number }[]> {
const { fileId, limit = 8, minSimilarity = 0.25, spaceKey } = options;
if (!spaceKey) return [];
if (this.useNativeVec) {
try {
const nativeResults = this.searchSimilarPersonalNative(queryEmbedding, fileId, limit, minSimilarity, spaceKey);
if (nativeResults.length > 0) return nativeResults;
} catch (e) {
console.warn('[VectorStore] Native personal vec search failed; using JS cosine fallback:', e);
}
}
return this.searchSimilarPersonalJS(queryEmbedding, fileId, limit, minSimilarity, spaceKey);
}

private searchSimilarPersonalNative(
queryEmbedding: number[], fileId: string | undefined, limit: number, minSimilarity: number, spaceKey: string,
): { chunkId: string; fileId: string; similarity: number }[] {
const queryBlob = this.embeddingToBlob(queryEmbedding);
const dim = queryEmbedding.length;
DatabaseManager.getInstance().ensurePersonalVecTableForDim(dim);
const fetchLimit = limit * 4;
const vecRows = this.db.prepare(`
SELECT personal_chunk_rowid, distance
FROM vec_personal_chunks_${dim}
WHERE embedding MATCH ?
ORDER BY distance
LIMIT ?
`).all(queryBlob, fetchLimit) as any[];
if (!vecRows.length) return [];
const ids = vecRows.map((row) => row.personal_chunk_rowid);
const placeholders = ids.map(() => '?').join(',');
let sql = `
SELECT pc.rowid AS personal_chunk_rowid, pc.id AS chunk_id, pc.file_id
FROM personal_file_chunks pc
WHERE pc.rowid IN (${placeholders})
AND pc.embedding_space = ?
`;
const params: any[] = [...ids, spaceKey];
if (fileId) { sql += ' AND pc.file_id = ?'; params.push(fileId); }
const rows = this.db.prepare(sql).all(...params) as any[];
const byRowid = new Map<number, any>();
for (const row of rows) byRowid.set(Number(row.personal_chunk_rowid), row);
const results: { chunkId: string; fileId: string; similarity: number }[] = [];
for (const vecRow of vecRows) {
const row = byRowid.get(Number(vecRow.personal_chunk_rowid));
if (!row) continue;
const similarity = 1 - Number(vecRow.distance);
if (similarity >= minSimilarity) results.push({ chunkId: row.chunk_id, fileId: row.file_id, similarity });
}
return results.slice(0, limit);
}

private searchSimilarPersonalJS(
queryEmbedding: number[], fileId: string | undefined, limit: number, minSimilarity: number, spaceKey: string,
): { chunkId: string; fileId: string; similarity: number }[] {
let sql = `SELECT id AS chunk_id, file_id, embedding FROM personal_file_chunks WHERE embedding IS NOT NULL AND embedding_space = ?`;
const params: any[] = [spaceKey];
if (fileId) { sql += ' AND file_id = ?'; params.push(fileId); }
const rows = this.db.prepare(sql).all(...params) as any[];
const dim = queryEmbedding.length;
...
/** Remove all Personal File vectors for one file before its chunks are replaced/deleted. */
deletePersonalEmbeddingsForFile(fileId: string): void {
try {
if (this.useNativeVec) {
const rows = this.db.prepare('SELECT rowid FROM personal_file_chunks WHERE file_id = ?').all(fileId) as Array<{ rowid: number }>;
if (rows.length) {
const placeholders = rows.map(() => '?').join(',');
const ids = rows.map((row) => row.rowid);
for (const dim of DatabaseManager.getInstance().getExistingPersonalVecDims()) {
try { this.db.prepare(`DELETE FROM vec_personal_chunks_${dim} WHERE personal_chunk_rowid IN (${placeholders})`).run(...ids); }
catch (_) { /* dimension table may not exist */ }
}
}
}
this.db.prepare(`
UPDATE personal_file_chunks
SET embedding = NULL, embedding_provider = NULL, embedding_dimensions = NULL, embedding_space = NULL
WHERE file_id = ?
`).run(fileId);
} catch (e) {
console.warn(`[VectorStore] Failed to clear personal embeddings for file ${fileId}:`, e);
}
}
